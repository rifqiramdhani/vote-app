...
export default function CandidateForm(props: Props) {
    ...
    return (
        <div className='flex flex-col border border-zinc-100 p-5'>
            ...
            <h1 className='flex w-1/2 bg-zinc-100 aspect-square selft-center text-center text-4xl rounded-full justify-center items-center'>{props.candidate.key}</h1>
            <label className='text-sm mt-3 mb-1'>Nama Kandidat</label>
           
            <Form placeholder='Masukkan Nama Kandidat' value={candidate.name} onChange={(e) => { setCandidate({...candidate,name: e}) }} />
        </div>
    )
}
